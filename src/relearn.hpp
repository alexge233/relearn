...
void q_learning<S,A>::operator()(episode<S,A> & arg, float gamma, float alpha)
{
    ...
    //          - so for each forward iteration we do a breadth-first search
    //          - until we find the reward
    //          - whilst indexing policies
    //          - once we have the reward, we will update the policies
    //          - and then insert them or update them in the episode
